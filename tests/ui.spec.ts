...
test('human can preserve a branch intention without visiting new territory',async({page,request})=>{await page.goto('/?noDraft=1');const view=page.frameLocator('#view');await view.getByRole('button',{name:'Branch from here'}).click();await view.getByLabel('Path name').fill('Follow the blind spot');await view.getByLabel('What should the next walk attend to?').fill('Attend to what the current framing keeps excluding.');await view.getByRole('button',{name:'Save path'}).click();await expect(view.getByRole('status')).toContainText('Path saved');const id=await page.locator('body').getAttribute('data-exploration-id');const snapshot=await (await request.get(`/snapshot?id=${id}`)).json();expect(snapshot.positions).toHaveLength(1);expect(snapshot.activeMove).toBeNull();expect(snapshot.cartography.lines).toHaveLength(2);expect(snapshot.cartography.gestureRequests.at(-1).kind).toBe('branch');await expect(view.getByRole('heading',{name:'Ready when you return to chat'})).toBeVisible();await expect(view.getByText('Resume the saved path “Follow the blind spot”.')).toBeVisible();});
test('human can request a weave across developed lines without creating an encounter',async({page,request})=>{await page.goto('/?flightLines=1&noDraft=1');const view=page.frameLocator('#view');await view.getByRole('button',{name:/Compare paths/}).click();const choices=view.locator('.line-choice input:not(:disabled)');expect(await choices.count()).toBeGreaterThanOrEqual(2);for(let i=0;i<await choices.count();i++)await choices.nth(i).check();await view.getByLabel('What do you want to compare?').fill('Do these lines preserve the same invariant, or merely use similar language?');const id=await page.locator('body').getAttribute('data-exploration-id');const before=await (await request.get(`/snapshot?id=${id}`)).json();await view.getByRole('button',{name:'Save comparison'}).click();await expect(view.getByRole('status')).toContainText('Comparison saved');const after=await (await request.get(`/snapshot?id=${id}`)).json();expect(after.cartography.encounters).toHaveLength(before.cartography.encounters.length);expect(after.cartography.gestureRequests.at(-1).kind).toBe('weave');expect(after.cartography.weaveProposals).toHaveLength(0);});
test('human review is required before a conversational weave becomes an encounter',async({page,request})=>{await page.goto('/?flightLines=1&weaveReview=1');const view=page.frameLocator('#view');await expect(view.getByText('PROPOSED RELATIONSHIP · WEAVE · REVISION 1')).toBeVisible();await expect(view.getByRole('button',{name:'Keep relationship'})).toBeEnabled();const id=await page.locator('body').getAttribute('data-exploration-id');const before=await (await request.get(`/snapshot?id=${id}`)).json();await view.getByRole('button',{name:'Keep relationship'}).click();await expect(view.getByRole('status')).toContainText('Comparison kept as a candidate encounter');const after=await (await request.get(`/snapshot?id=${id}`)).json();expect(after.cartography.encounters).toHaveLength(before.cartography.encounters.length+1);expect(after.cartography.weaveProposals.at(-1).status).toBe('kept');expect(after.activeMove).toBeNull();});
