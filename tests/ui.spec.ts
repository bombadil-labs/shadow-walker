...
test('saved branch persists and renders a saved direction',async({page,request})=>{
  ...
  const id=await page.locator('body').getAttribute('data-exploration-id');const snapshot=await (await request.get(`/snapshot?id=${id}`)).json();
  ...
  await expect(view.locator('.saved-directions-card')).toBeVisible();
});
