...
test('held branches and pending weaves are visible directly on the map',async({page})=>{
  ...
  await view.getByRole('button',{name:'Open a branch from this arrival'}).click();
  await view.getByLabel('Line name').fill('Keep the alternate boundary');
  await view.getByLabel('Direction for the next walk').fill('Follow the alternate boundary without collapsing it into the first line.');
  await view.getByRole('button',{name:'Save branch intention'}).click();
  ...
  await woven.getByRole('button',{name:'Bring lines into encounter'}).click();
  const choices=woven.locator('.line-choice input:not(:disabled)');for(let i=0;i<await choices.count();i++)await choices.nth(i).check();
  await woven.getByLabel('What should the encounter test?').fill('Do the developed lines preserve the same authority invariant?');
  await woven.getByRole('button',{name:'Save weave intention'}).click();
