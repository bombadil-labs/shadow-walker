...
export class Store {
  ...
  constructor(path = ':memory:', now: () => number = Date.now) {
    ...
    // Compatibility projection: legacy M1 positions become visited Arrivals on one explicit line.
    // We never fabricate semantic-shift metadata that was not recorded at the time.
  ...
  private bootstrapCartography(): void {
    ...
    for (const row of explorations) {
      ...
      const existing = this.db.prepare('SELECT id FROM lines WHERE exploration_id=? LIMIT 1').get(exploration.id);
      if (existing) continue;
      const line: Line = { id: `line:${exploration.id}:legacy`, explorationId: exploration.id, label: 'Original line',
        status: 'active', originPositionId: exploration.rootId, createdAt: exploration.createdAt };
      ...
      for (const item of positions) {
        ...
        this.db.prepare('INSERT OR IGNORE INTO line_memberships(line_id,position_id,role,created_at) VALUES(?,?,?,?)')
          .run(line.id, position.id, position.id === exploration.rootId ? 'origin' : 'arrival', position.createdAt);
        for (const parent of position.parentIds) {
          if (!ids.has(parent)) continue;
          const transition: Transition = { id: `transition:${parent}:${position.id}`, explorationId: exploration.id,
            fromPositionId: parent, toPositionId: position.id, lineId: line.id, kind: position.kind === 'excavation' ? 'excavated' : 'walked-to', createdAt: position.createdAt };
          this.db.prepare('INSERT OR IGNORE INTO transitions(id,exploration_id,from_position_id,to_position_id,line_id,kind,body) VALUES(?,?,?,?,?,?,?)')
            .run(transition.id, exploration.id, parent, position.id, line.id, transition.kind, JSON.stringify(transition));
        }
      }
    }
  }
  private cartography(explorationId: string): CartographySnapshot {
    ...
    return {lines,memberships,transitions};
  }
  private resolveLine(explorationId: string, selectedIds: string[], requested?: string): Line {
    ...
    if (requested) {
      const line = candidates.find(line=>line.id===requested);
      requireThat(line,'INVALID_LINE','The requested line does not contain every selected arrival.'); return line;
    }
  ...
  private addTransition(line: Line, from: string, to: Position): void {
    const transition: Transition = { id: randomUUID(), explorationId: to.explorationId, fromPositionId: from, toPositionId: to.id,
      lineId: line.id, kind: to.kind === 'excavation' ? 'excavated' : 'walked-to', createdAt: to.createdAt };
    this.db.prepare('INSERT INTO transitions(id,exploration_id,from_position_id,to_position_id,line_id,kind,body) VALUES(?,?,?,?,?,?,?)')
      .run(transition.id,to.explorationId,from,to.id,line.id,transition.kind,JSON.stringify(transition));
  }
  ...
  create(input: { title: string; intention: string; frame: {label: string; constraints: string[]}; requestId: string }): Snapshot {
    ...
    return this.transaction(() => this.receipt('create', input.requestId, input, () => {
      ...
      const line: Line = { id: randomUUID(), explorationId: id, label: 'First line', status: 'active', originPositionId: rootId, createdAt };
      this.db.prepare('INSERT INTO lines(id,exploration_id,body) VALUES(?,?,?)').run(line.id,id,JSON.stringify(line));
      this.addMembership(line,root,'origin');
  ...
  forkLine(input: { explorationId: string; fromPositionId: string; label: string; requestId: string }): Snapshot {
    text(input.label,'line label',200);
    return this.transaction(() => this.receipt(`fork:${input.explorationId}`,input.requestId,input,()=>{
      const s=this.read(input.explorationId); const origin=s.positions.find(p=>p.id===input.fromPositionId);
      requireThat(origin,'NOT_FOUND','The branch origin is not an arrival in this exploration.');
      ...
      this.addMembership(line,origin,'origin'); this.event(line.explorationId,'line.forked',{line,fromPositionId:origin.id}); return this.read(line.explorationId);
    }));
  }
  prepare(input: { explorationId: string; selectedIds: string[]; humanDirection: string; requestId: string; lineId?: string }): MovePacket {
    ...
    return this.transaction(() => this.receipt(`prepare:${input.explorationId}`, input.requestId, input, () => {
      ...
      const packet = boundMoveContext({
        protocolVersion: '0.2', moveId: randomUUID(), kind: 'walk', explorationId: input.explorationId, line,
  ...
  submit(input: { moveId: string; output: MoveOutput; requestId: string }): Draft {
    return this.transaction(() => this.receipt(`submit:${input.moveId}`, input.requestId, input, () => {
      ...
      validateOutput(input.output,{requireSemanticShift:packet.protocolVersion==='0.2'});
      ...
      this.fresh(packet, s); orderProposals(input.output, s.positions, packet.selectedInputs.map(p => p.id),packet.protocolVersion==='0.2');
  ...
  review(input: ReviewInput): Snapshot {
    ...
    return this.transaction(() => this.receipt(`review:${input.draftId}`, input.requestId, fingerprint, () => {
      ...
      if (input.action === 'revise') {
        orderProposals(input.output!, s.positions, packet.selectedInputs.map(p => p.id),packet.protocolVersion==='0.2');
        ...
        const ordered = orderProposals(draft.output, s.positions, packet.selectedInputs.map(p => p.id),packet.protocolVersion==='0.2');
        const ids = new Map(ordered.map(p => [`draft:${p.localId}`, randomUUID()]));
        const landed = ordered.map(({localId, ...p}): Position => {
          const parentIds=p.parentIds.map(id=>ids.get(id)??id);
          const semanticShift=p.semanticShift?{...p.semanticShift,baselineArrivalIds:p.semanticShift.baselineArrivalIds.map(id=>ids.get(id)??id)}:undefined;
          return {...p,semanticShift,id:ids.get(`draft:${localId}`)!,explorationId:draft.explorationId,parentIds,originMoveId:draft.moveId,acceptance:'accepted',epistemicStatus:'hypothesis',createdAt:this.stamp()};
        });
        const line=packet.line??this.resolveLine(packet.explorationId,packet.selectedInputs.map(p=>p.id));
        for (const p of landed) {
          this.db.prepare('INSERT INTO positions(id,exploration_id,body) VALUES(?,?,?)').run(p.id, p.explorationId, JSON.stringify(p));
          this.addMembership(line,p);
        }
        for(const p of landed)for(const parent of p.parentIds)this.addTransition(line,parent,p);
        ...
        draft.status = 'landed'; this.event(draft.explorationId, 'positions.landed', { draftId: draft.id, positions: landed, lineId: line.id });
