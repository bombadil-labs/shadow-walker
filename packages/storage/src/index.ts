...
import type { CartographySnapshot, Draft, Exploration, Frame, Line, LineMembership, MoveOutput, MovePacket, Position, ReviewInput, ReviewTicket, Snapshot, Transition, Waypoint } from '../../domain/src/index.ts';
...
export class Store {
  ...
  private cartography(explorationId:string):CartographySnapshot{
    ...
    const waypoints=(this.db.prepare('SELECT body FROM waypoints WHERE exploration_id=? ORDER BY rowid').all(explorationId) as Row[]).map(r=>JSON.parse(r.body) as Waypoint);
    return {lines,memberships,transitions,waypoints,...this.flightLines.snapshot(explorationId)};
  }
  ...
  private addWaypoint(input:{explorationId:string;fromPositionId:string;question:string;provenance:Waypoint['provenance']}):Waypoint{
    text(input.question,'waypoint question');
    requireThat(['human-offered','walker-sensed','breakdown-emergent','operation-adjacent','resonance-detected'].includes(input.provenance),'INVALID_INPUT','Invalid waypoint provenance.');
    const position=this.db.prepare('SELECT id FROM positions WHERE id=? AND exploration_id=?').get(input.fromPositionId,input.explorationId);
    requireThat(position,'INVALID_PARENT','Waypoint origin must be a visited arrival in this exploration.');
    const waypoint:Waypoint={id:randomUUID(),explorationId:input.explorationId,fromPositionId:input.fromPositionId,question:input.question,provenance:input.provenance,status:'sensed',createdAt:this.stamp()};
    this.db.prepare('INSERT INTO waypoints(id,exploration_id,from_position_id,status,body) VALUES(?,?,?,?,?)').run(waypoint.id,waypoint.explorationId,waypoint.fromPositionId,waypoint.status,JSON.stringify(waypoint));
    this.event(waypoint.explorationId,'waypoint.sensed',waypoint);return waypoint;
  }
  private loadWaypoint(explorationId:string,waypointId:string):Waypoint{
    text(waypointId,'waypointId',128);
    const row=this.db.prepare('SELECT body FROM waypoints WHERE id=? AND exploration_id=?').get(waypointId,explorationId) as Row|undefined;
    requireThat(row,'NOT_FOUND','Waypoint is not in this exploration.');return JSON.parse(row.body) as Waypoint;
  }
  private resolveWaypointVisited(waypoint:Waypoint,visitedPositionId:string):Waypoint{
    requireThat(waypoint.status==='sensed','WAYPOINT_CLOSED','This route is no longer an open direction.');
    const updated:Waypoint={...waypoint,status:'visited',visitedPositionId,resolvedAt:this.stamp()};
    this.db.prepare('UPDATE waypoints SET status=?,body=? WHERE id=?').run(updated.status,JSON.stringify(updated),updated.id);
    this.event(updated.explorationId,'waypoint.visited',updated);return updated;
  }
  ...
  recordWaypoint(input:{explorationId:string;fromPositionId:string;question:string;provenance:Waypoint['provenance'];requestId:string}):Snapshot{
    return this.transaction(()=>this.receipt(`waypoint:${input.explorationId}`,input.requestId,input,()=>{
      const exploration=this.load<Exploration>('explorations',input.explorationId);this.addWaypoint(input);
      exploration.revision++;this.db.prepare('UPDATE explorations SET body=? WHERE id=?').run(JSON.stringify(exploration),exploration.id);
      return this.read(input.explorationId);
    }));
  }
  dissipateWaypoint(input:{explorationId:string;waypointId:string;reason:string;requestId:string}):Snapshot{
    text(input.reason,'waypoint dissipation reason',2000);
    return this.transaction(()=>this.receipt(`waypoint-dissipate:${input.explorationId}`,input.requestId,input,()=>{
      const exploration=this.load<Exploration>('explorations',input.explorationId),waypoint=this.loadWaypoint(input.explorationId,input.waypointId);
      requireThat(waypoint.status==='sensed','WAYPOINT_CLOSED','Only a sensed route can dissipate.');
      const updated:Waypoint={...waypoint,status:'dissipated',resolvedAt:this.stamp(),resolutionNote:input.reason};
      this.db.prepare('UPDATE waypoints SET status=?,body=? WHERE id=?').run(updated.status,JSON.stringify(updated),updated.id);
      exploration.revision++;this.db.prepare('UPDATE explorations SET body=? WHERE id=?').run(JSON.stringify(exploration),exploration.id);
      this.event(updated.explorationId,'waypoint.dissipated',updated);return this.read(input.explorationId);
    }));
  }
  ...
  prepare(input:{explorationId:string;selectedIds:string[];humanDirection:string;requestId:string;lineId?:string;waypointId?:string}):MovePacket{
    ...
    return this.transaction(()=>this.receipt(`prepare:${input.explorationId}`,input.requestId,input,()=>{
      ...
      const routeWaypoint=input.waypointId?this.loadWaypoint(input.explorationId,input.waypointId):undefined;
      if(routeWaypoint){requireThat(routeWaypoint.status==='sensed','WAYPOINT_CLOSED','Only a sensed route can be followed.');requireThat(input.selectedIds.includes(routeWaypoint.fromPositionId),'INVALID_WAYPOINT','A followed route must originate from one of the selected arrivals.');}
      const packet=boundMoveContext({protocolVersion:'0.2',moveId:randomUUID(),kind:'walk',explorationId:input.explorationId,line,...(routeWaypoint?{routeWaypoint}:{}),frame:s.exploration.frame,originalIntention:s.exploration.intention,selectedInputs:selected,priorRecordedWaypoint:s.positions[s.positions.length-1]!,humanDirection:input.humanDirection,dependencyVersions:{exploration:s.exploration.revision,frame:s.exploration.frame.version},budget:{maxMoves:1,maxPositions:2},instructions:[
        ...
        ...(routeWaypoint?[`You are explicitly following the sensed route: "${routeWaypoint.question}". Let current human direction refine it without pretending the destination was already mapped.`]:[]),
  ...
  review(input:ReviewInput):Snapshot{
    ...
    return this.transaction(()=>this.receipt(`review:${input.draftId}`,input.requestId,fingerprint,()=>{
      ...
      if(input.action==='revise'){orderProposals(input.output!,s.positions,packet.selectedInputs.map(p=>p.id),packet.protocolVersion==='0.2');draft.output=input.output!;draft.status='pending';this.db.prepare("UPDATE moves SET status='submitted' WHERE id=?").run(draft.moveId);}
        ...
        if(packet.routeWaypoint&&landed[0])this.resolveWaypointVisited(this.loadWaypoint(packet.explorationId,packet.routeWaypoint.id),landed[0].id);
        for(const p of landed)this.addWaypoint({explorationId:p.explorationId,fromPositionId:p.id,question:p.nextQuestion,provenance:'walker-sensed'});
        s.exploration.revision++;this.db.prepare('UPDATE explorations SET body=? WHERE id=?').run(JSON.stringify(s.exploration),draft.explorationId);draft.status='landed';this.event(draft.explorationId,'positions.landed',{draftId:draft.id,positions:landed,lineId:line.id,followedWaypointId:packet.routeWaypoint?.id});
