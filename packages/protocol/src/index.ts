...
export const proposal = z.object({localId: z.string().regex(/^[a-zA-Z][a-zA-Z0-9_-]*$/).max(64),
  ...
  uncertainty: strings.min(1), nextQuestion: text, semanticShift:semanticShift.optional()}).strict();
...
const cartography=z.object({lines:z.array(line),memberships:z.array(membership),transitions:z.array(transition)}).strict();
...
const packet = z.object({protocolVersion:z.enum(['0.1','0.2']),moveId:id,kind:z.literal('walk'),explorationId:id,line:line.optional(),frame,originalIntention:text,
  ...
  outputContract:z.object({kinds:z.tuple([z.literal('excavation'),z.literal('question')]),localParentPrefix:z.literal('draft:'),requiresSemanticShift:z.literal(true).optional()}).strict()}).strict();
