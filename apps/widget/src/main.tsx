...
function ArrivalDetail({node}:{node?:VisualNode}){
  ...
  return <section className="detail-card"><p className="eyebrow">{node.type==='draft'?(node.draftStatus==='reserved'?'SAVED FOR LATER':'PROPOSED STEP'):'VISITED ARRIVAL'}</p><h2>{source.meaning}</h2>
    {shift?<><p className="shift-summary">{shift.summary}</p><ShiftChips shift={shift}/>{shift.receded.length>0&&<div className="detail-block"><h3>What receded</h3><p>{shift.receded.map(s=>s.span).join(' · ')}</p></div>}
      ...
      {shift.unexpectedConnections.length>0&&<div className="detail-block"><h3>Unexpected connections</h3><ul>{shift.unexpectedConnections.map((v,i)=><li key={i}>{v}</li>)}</ul></div>}</>:<p className="legacy-note">This arrival predates semantic-shift recording. Shadow Walker will not invent a historical shift after the fact.</p>}
...
function Workbench(){
  ...
  const [error,setError]=useState('');const [notice,setNotice]=useState('');const [busy,setBusy]=useState(false);const retry=useRef<{signature:string;requestId:string}|undefined>();
  ...
  function receive(result:CallToolResult){
    ...
    if(d)setSelected(`proposal:${d.id}:${d.output.positions[0]?.localId}`);else if(!selected)setSelected(data.snapshot.positions.at(-1)?.id);
  }
